...
export const interpolateFString = (template: string, values: InputValues) =>
  ...
  type Intepolator = (template: string, values: InputValues) => string;
  
  type Parser = (template: string) => ParsedFStringNode[];

  export const DEFAULT_FORMATER_MAPPING: Record<TemplateFormat, Intepolator> = {
  ...
  ) => DEFAULT_FORMATER_MAPPING[templateFormat](template, InputValues);
  ...
  export const checkValidTemplate = (
    ...
    if (!(templateFormat in DEFAULT_FORMATER_MAPPING)) {
      const validFormats = Object.keys(DEFAULT_FORMATER_MAPPING);
